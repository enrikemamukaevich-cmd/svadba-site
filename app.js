...
function plusTap() {
  var stop = uploadStop();
  if (!stop) { openUpload(); return; }

  /* Кнопка считает, что приём закрыт. Прежде чем отказать, переспрашиваем базу:
     настройки в браузере могли устареть в сторону запрета. Владелец выключил
     рубильник, потом включил обратно, а страница гостя висит открытой с утра —
     без этой проверки такой гость до перезагрузки так и будет получать
     «Загрузка сейчас выключена». Обратное направление ловится и без нас:
     openUpload и sendItem перечитывают настройки сами.
     Отказ стоит денег в одну лишнюю строчку ответа, и только по нажатию. */
  readSettings().then(function () {
    var again = uploadStop();
    applyState();
    if (again === 'closed') return;      // applyState уже увёл на страницу благодарности
